...
export async function convertAddressToCoord(address: Address): Promise<Coordinates> {
  const params = new URLSearchParams({
    ...Object.fromEntries(Object.entries(address).filter(([_, v]) => v !== undefined)),
    format: 'json',
  });
  const response = await fetch(`https://nominatim.openstreetmap.org/search?${params}`, {
    headers: { 'user-agent': 'testapp/0.1 (hawkeye.zach@gmail.com)' },
  });
...
export async function getDirections(
  ...
  return coords.map((crd: number[]) => ({ long: crd[0], lat: crd[1] }));
}
