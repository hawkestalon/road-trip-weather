...
async function lookupCoordinate(location: Coordinates): Promise<GridInfo> {
  const response = await fetch(`https://api.weather.gov/points/${location.lat},${location.long}`, {headers: { 'User-Agent': '(weatherApp/0.1 hawkeye.zach@gmail.com)'}})
...
async function getWeatherForGridPoint(gridPt: GridInfo) {
  const response = await fetch(`https://api.weather.gov/gridpoints/${gridPt.station}/${gridPt.grid.x},${gridPt.grid.y}/forecast/hourly`, {headers: { 'User-Agent': '(weatherApp/0.1 hawkeye.zach@gmail.com)'}})
  const res = await response.json();
